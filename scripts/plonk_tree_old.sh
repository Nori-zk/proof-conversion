...
ENV=${args[0]}
source ${ENV}
RAYON_NUM_THREADS=4  # Per-process parallelism
NODE_MEMORY_LIMIT=6144  # 6GB per Node.js process
...
for i in `seq 0 $MAX_ITERATIONS`; do
  for j in `seq 0 $(( $MAX_THREADS - 1 ))`; do
    ...
    numactl --cpunodebind=$NUMA_NODE --membind=$NUMA_NODE node --max-old-space-size=$NODE_MEMORY_LIMIT ./build/src/plonk/recursion/prove_zkps.js zkp${ZKP_I} $HEX_PROOF $PROGRAM_VK $HEX_PI $AUX_WITNESS_RELATIVE_PATH ${WORK_DIR_RELATIVE_TO_SCRIPTS} ${CACHE_DIR_RELATIVE_TO_SCRIPTS} &
  done
  wait
  if $SHOULD_BREAK; then
    break
...
for i in `seq 1 5`; do
    ...
    for j in `seq 0 $MAX_ITERATIONS`; do
       ...
       for k in `seq 0 $(( $MAX_THREADS - 1 ))`; do
          ...
          # echo "${i}, ${j}, ${k}, ${ZKP_J}"
          # numactl --cpunodebind=$NUMA_NODE --membind=$NUMA_NODE  node ./build/src/node_resolver.js --max-old-space-size=$NODE_MEMORY_LIMIT  $TOTAL_IN_LOOP ${i} ${ZKP_J} ${WORK_DIR_RELATIVE_TO_SCRIPTS} ${CACHE_DIR_RELATIVE_TO_SCRIPTS} &
