...
export class HelloWorldRollup extends SmartContract {
  ...
  @method async setParameters(
    admin: PrivateKey,
    blobstreamAddress: PublicKey,
  ...
  @method async update(
    admin: PrivateKey,
    pathInBlobstream: BlobstreamMerkleWitness,
    batcherProof: BatcherProofType
  ) {
    const blobstreamAddress = this.blobstreamAddress.get();
    this.blobstreamAddress.requireEquals(blobstreamAddress);

    const blobstreamContract = new BlobstreamProcessor(
      this.blobstreamAddress.get()
    );
    ...
    pathInBlobstream
      .calculateRoot(
        Poseidon.hash([...batcherProof.publicOutput.dataCommitment.toFields()])
      )
    ...
    blobInclusionVkHash.assertEquals(
      batcherProof.publicInput.blobInclusionVkHash
    );
