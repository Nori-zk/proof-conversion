...
const layer1Vk = (await layer1.compile({ cache: Cache.FileSystem(cacheDir) }))
  .verificationKey;
fs.writeFileSync(
  `${workDir}/vks/layer1Vk.json`,
  JSON.stringify(layer1Vk),
  'utf8'
);

const nodeVk = (await node.compile({ cache: Cache.FileSystem(cacheDir) }))
  .verificationKey;
